...
if __name__ == '__main__':
    ...
    parser.add_argument('--number_workers', '-nw', '--num_workers', type=int, default=16)
    parser.add_argument('--number_gpus', '-ng', type=int, default=4, help='number of GPUs to use')
    ...
    parser.add_argument('--resume', default='./checkpoint/FlowNet2_checkpoint.pth.tar', type=str, metavar='PATH', help='path to latest checkpoint (default: none)')
    ...
    tools.add_arguments_for_module(parser, losses, argument_for_class='loss', default='L3Loss')
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        if True:
            train_list = "./img_list/train.list"
            train_dataset = datasets.MpiSintel(args, train_list, is_cropped = False, root = '', dstype = 'final', replicates = 1)
        ...
        if True:
            test_list = "./img_list/test.list"
            validation_dataset = datasets.MpiSintel(args, test_list, is_cropped = False, root = '', dstype = 'clean', replicates = 1)
        ...
        if True:
            train_list = "./img_list/train.list"
            inference_dataset =datasets.MpiSintel(args, train_list, is_cropped = False, root = '', dstype = 'clean', replicates = 1)
    ...
    with tools.TimerBlock("Building {} model".format(args.model)) as block:
        class ModelAndLoss(nn.Module):
            def __init__(self, args):
                ...
                self.resnet_model = resnet34(pretrained=True, channel=CHANNEL).cuda()
    ...
    def train(args, epoch, start_iteration, data_loader, model, optimizer, logger, is_validate=False, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            losses, logits = model(data[0], target[0])
            ...
            logits = logits.cpu().long()
        
            batch_correct = (torch.max(logits, 1)[1].view(target[0].size()).data == target[0].data).sum()

            batch_accuracy = 100.0*batch_correct/target[0].size()[0]
            average_accuracy += batch_accuracy
            
           
            print('\033[91m' +"batch accuracy    "+str(batch_accuracy)+ '\033[0m') 
